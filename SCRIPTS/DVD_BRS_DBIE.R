...
system(" mkdir /home/aakumar/WORK/LINK_ANAL/DVD_BRS/DBIE_inputfiles") ## Add the parametr

Data<-read.table("/home/aakumar/WORK/LINK_ANAL/DVD_BRS/DATA/Full_Data_Table_22_indiv.txt",header=T,sep='\t',dec=",",na.strings="NC",stringsAsFactors=F,fill=T) ## Add the input parameter
root<-Data[,1:10] 

### Structure for the Genptype DATA file should be made as #################33

### Illumia platform data file ###############

# DBIE
dbie.cols<-grep("^DBIE",names(Data)) ## Input parameter  #Column heading of the genotype information
data.dbie<-as.data.frame(cbind(root,Data[,dbie.cols]))
Data = data.dbie

#snp.chr = table(Data$Chr)[-c(23:26)]
snp.chr = table(Data$Chr)[-c(1,24:27)] # Find the frequency of the genotype
...
### Print to the screen about how many rs/SNP ids are present #################
...
#interval<-300000
#interval<-200000
interval<-450000 # Intermarker distance
#interval <-50000
...
for(i in 1:n.chr){
  ...
  #write.table(mapfile[[i]],paste("DBIE_inputfiles/map.",i,sep=""), quote=F,row.names=F,sep="\t")
  write.table(mapfile[[i]],paste("/home/aakumar/WORK/LINK_ANAL/DVD_BRS/DBIE_inputfiles/map.",i,sep=""), quote=F,row.names=F,sep="\t") ## Adjust accordingly and automatically 
}
...
for (i in 1:n.chr){
  ...
  write.table(namesfile[[i]],paste("/home/aakumar/WORK/LINK_ANAL/DVD_BRS/DBIE_inputfiles/names.",i,sep=""), quote=F,row.names=F,col.names=F,sep="\t")
}
...
#raw.pre<-read.table("/home/aakumar/WORK/LINK_ANAL/DVD_BRS/PED/DBIE_Ped_file.csv",header=F,sep="\t")
raw.pre<-read.table("/home/aakumar/WORK/LINK_ANAL/DVD_BRS/PED/DBIE_Ped_file_2.csv",header=F,sep="\t") ## Pedigree file 
#pre.6<-raw.pre[,-7]
...
############## Add automated way to extract from the PED file ######################
DBIE_11 <-matrix(NA,nrow=n.snp,ncol=1)
DBIE_12 <-matrix(NA,nrow=n.snp,ncol=1)
DBIE_21 <-matrix(NA,nrow=n.snp,ncol=1)
DBIE_23 <-matrix(NA,nrow=n.snp,ncol=1) #
DBIE_24 <-matrix(NA,nrow=n.snp,ncol=1)
DBIE_25 <-matrix(NA,nrow=n.snp,ncol=1) #
DBIE_26 <-matrix(NA,nrow=n.snp,ncol=1)
DBIE_27 <-matrix(NA,nrow=n.snp,ncol=1)
DBIE_28 <-matrix(NA,nrow=n.snp,ncol=1) #
DBIE_31 <-matrix(NA,nrow=n.snp,ncol=1) #
DBIE_32 <-matrix(NA,nrow=n.snp,ncol=1) #
DBIE_35 <-matrix(NA,nrow=n.snp,ncol=1) #
DBIE_36 <-matrix(NA,nrow=n.snp,ncol=1) #
DBIE_37 <-matrix(NA,nrow=n.snp,ncol=1) #
DBIE_38 <-matrix(NA,nrow=n.snp,ncol=1) #
DBIE_39 <-matrix(NA,nrow=n.snp,ncol=1) #
DBIE_210 <-matrix(NA,nrow=n.snp,ncol=1)
DBIE_211 <-matrix(NA,nrow=n.snp,ncol=1) #
DBIE_212 <-matrix(NA,nrow=n.snp,ncol=1)
DBIE_311 <-matrix(NA,nrow=n.snp,ncol=1)



#empty<-as.data.frame(cbind(KIV_000025,KIV_000028))
empty<-as.data.frame(cbind(DBIE_11, DBIE_12, DBIE_21, DBIE_23, DBIE_24, DBIE_25, DBIE_26,DBIE_27, DBIE_28,DBIE_31,DBIE_32,DBIE_35,DBIE_36,DBIE_37, DBIE_38,DBIE_39, DBIE_210,DBIE_211, DBIE_212,DBIE_311))
#names(empty)<-c("KIV_000025","KIV_000028")
#names(empty)<-c("DBIE_11","DBIE_12","DBIE_21","DBIE_24","DBIE_26","DBIE_27","DBIE_210","DBIE_212","DBIE_311")
names(empty)<-c("DBIE_11", "DBIE_12","DBIE_21", "DBIE_23", "DBIE_24", "DBIE_25", "DBIE_26","DBIE_27", "DBIE_28","DBIE_31","DBIE_32","DBIE_35","DBIE_36","DBIE_37", "DBIE_38","DBIE_39", "DBIE_210","DBIE_211", "DBIE_212","DBIE_311")
...
# recodeer genotype naar 0.1.2 ### AA =>0 ; AB => 1; BB => 2
...
for (i  in 1:n.chr){
	...
	write.table(prefile[[i]],file=paste("/home/aakumar/WORK/LINK_ANAL/DVD_BRS/DBIE_inputfiles/pedin.",i,sep=""),sep="\t", quote=F,row.names=F,col.names=F,na="0")
}

system("cp -r /home/aakumar/WORK/LINK_ANAL/DVD_BRS/DBIE_inputfiles  /home/aakumar/WORK/LINK_ANAL/DVD_BRS/ORIG_DBIE_inputfiles")
system("cp /home/aakumar/WORK/LINK_ANAL/DVD_BRS/PAR/* /home/aakumar/WORK/LINK_ANAL/DVD_BRS/DBIE_inputfiles/")
