...
Data<-read.table("../Full_Data_Table.txt",header=T,sep='\t',dec=",",na.strings="NC",stringsAsFactors=F,fill=T)
root<-Data[,1:10]

# kiv
kiv.cols<-grep("^KIV",names(Data))
data.kiv<-as.data.frame(cbind(root,Data[,kiv.cols]))
Data = data.kiv
Data = Data[,-14]
...
####################################################################
# n.ID is the total number of array IDs present in the Data Variable
#*******************************************************************
n.ID<-18
n.col<-n.ID+3
...
write.table(mapfile[[i]],paste("inputfiles_2/map.",i,sep=""),
...
write.table(namesfile[[i]],paste("inputfiles_2/names.",i,sep=""),
...
raw.pre<-read.table("pedfile_branch_2_affected_only.txt",header=F,sep="\t")
#pre.6<-raw.pre[,-7]
...
#*********************************
# 
gt.1<-autosom[autosom$Name%in%Names.3$Name,-c(3,14:22)]
#gt.1<-autosom[autosom$Name%in%Names.3$Name,-c(3,15:22)]
...
n.snp<-dim(gt.1)[1]
KIV_000023<-matrix(NA,nrow=n.snp,ncol=1)
KIV_000024<-matrix(NA,nrow=n.snp,ncol=1)
KIV_000029<-matrix(NA,nrow=n.snp,ncol=1)
KIV_000030<-matrix(NA,nrow=n.snp,ncol=1)
KIV_000031<-matrix(NA,nrow=n.snp,ncol=1)


empty<-as.data.frame(cbind(KIV_000023,KIV_000024,KIV_000029,KIV_000030,KIV_000031))
names(empty)<-c("KIV_000023","KIV_000024","KIV_000029","KIV_000030","KIV_000031")
...
gt.melt$ID<-as.numeric(substr(gt.melt$variable,5,10))
...
write.table(prefile[[i]],file=paste("inputfiles_2/pedin.",i,sep=""),sep="\t",
quote=F,row.names=F,col.names=F,na="0")
}
