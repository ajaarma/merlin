...
system(" mkdir /home/aakumar/WORK/LINK_ANAL/DVD_BRS/KIV_inputfiles")

Data <-read.table("/home/aakumar/WORK/LINK_ANAL/KIV/Full_Data_Table.txt",header=T,sep='\t',dec=",",na.strings="NC",stringsAsFactors=F,fill=T)
root <-Data[,1:10]

# KIV
vass.cols <-grep("^KIV",names(Data))
data.vass <-as.data.frame(cbind(root,Data[,vass.cols]))
Data_1 = data.vass

Data_2 <-read.table("/home/aakumar/WORK/LINK_ANAL/DVD_BRS/DATA/Full_Data_Table_KIV_130562.txt",header=T,sep='\t',dec=",",na.strings="NC",stringsAsFactors=F,fill=T)
Data1_2 = merge(Data_2, Data_1,by.x="Name",by.y="Name")

ind = c(2,1,3,4,5,6,7,8,9,10,11,32:37,39)
Data = Data1_2[,ind]
colnames(Data)[1] <- "Index"
colnames(Data)[3] <- "Address"
colnames(Data)[4] <- "Chr"
colnames(Data)[5] <- "Position"
...
autosom <-autosom[o,]
#autosom <- autosom[,c(1:4,16:21,23)]

map.0<-autosom[,c(2,3,1)]
#map.0<-autosom[,c(3,2,1)]
...
for(i in 1:n.chr){
  ...
  #write.table(mapfile[[i]],paste("KIV_inputfiles/map.",i,sep=""), quote=F,row.names=F,sep="\t")
  write.table(mapfile[[i]],paste("/home/aakumar/WORK/LINK_ANAL/DVD_BRS/KIV_inputfiles/map.",i,sep=""), quote=F,row.names=F,sep="\t")
}
...
for (i in 1:n.chr){
  ...
  write.table(namesfile[[i]],paste("/home/aakumar/WORK/LINK_ANAL/DVD_BRS/KIV_inputfiles/names.",i,sep=""), quote=F,row.names=F,col.names=F,sep="\t")
}
...
raw.pre<-read.table("/home/aakumar/WORK/LINK_ANAL/DVD_BRS/PED/KIV_Ped_Fam1.csv",header=F,sep="\t")
pre.6<-raw.pre[,-c(7:8)]
...
#gt.1<-autosom[autosom$Name%in%Names.3$Name,-2]
...
KIV_000013<-matrix(NA,nrow=n.snp,ncol=1)
KIV_000014<-matrix(NA,nrow=n.snp,ncol=1)
KIV_000015<-matrix(NA,nrow=n.snp,ncol=1)
KIV_000016<-matrix(NA,nrow=n.snp,ncol=1)
KIV_000017<-matrix(NA,nrow=n.snp,ncol=1)
KIV_000018<-matrix(NA,nrow=n.snp,ncol=1)

empty<-as.data.frame(cbind(KIV_000013,KIV_000014,KIV_000015,KIV_000016,KIV_000017,KIV_000018))
names(empty)<-c("KIV_000013","KIV_000014","KIV_000015","KIV_000016","KIV_000017","KIV_000018")
...
gt.melt$ID<-(substr(gt.melt$variable,1,10)) #as.numeric
...
for (i  in 1:n.chr){
	...
	write.table(prefile[[i]],file=paste("/home/aakumar/WORK/LINK_ANAL/DVD_BRS/KIV_inputfiles/pedin.",i,sep=""),sep="\t", quote=F,row.names=F,col.names=F,na="0")
}

system("cp -r /home/aakumar/WORK/LINK_ANAL/DVD_BRS/KIV_inputfiles  /home/aakumar/WORK/LINK_ANAL/DVD_BRS/ORIG_KIV_inputfiles")
system("cp /home/aakumar/WORK/LINK_ANAL/DVD_BRS/PAR/* /home/aakumar/WORK/LINK_ANAL/DVD_BRS/KIV_inputfiles/")
